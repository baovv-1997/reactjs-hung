...
const ItemSubMenu = ({
  ...
  const activeSub =
    ...
    (location.pathname.includes(`merchandise/registerProduct`) &&
      keyId === 8) ||
    (location.pathname.includes(`management/pickup-order/${id}`) &&
      keyId === 10) ||
    (location.pathname.includes(`management/qa`) && keyId === 12) ||
    (location.pathname.includes(`management/notice`) && keyId === 13) ||
    (location.pathname.includes(`management/event-app`) && keyId === 11);
  return (
    <li
